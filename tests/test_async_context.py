...
if sys.version_info >= (3, 7):
    ...
    class TestAsyncContext(AsyncTestCase):
        ...
        def test_async_retry_state_context(self) -> None:
            ...
            and not globally.
            """
            async def _retry_immediately_coro(start_barrier: AsyncBarrier, end_barrier: AsyncBarrier) -> None:
            ...
            async def _no_retry_coro(start_barrier: AsyncBarrier, end_barrier: AsyncBarrier) -> None:
            ...
            async def _test_inner() -> None:
                ...
                await asyncio.gather(_no_retry_coro(start_barrier, end_barrier), _retry_immediately_coro(start_barrier, end_barrier))

                # retry state should not leak between tasks
                assert self.counter.most_common() == [('retry_immediately', 3), ('no_retry', 1)]
    ...
    class TestThreadedContext(unittest.TestCase):
        ...
        def test_threaded_retry_state_context(self) -> None:
            def _retry_immediately_inner(start_barrier: threading.Barrier, end_barrier: threading.Barrier) -> None:
                counter_key = 'retry_immediately'
            ...
            def _no_retry_inner(start_barrier: threading.Barrier, end_barrier: threading.Barrier) -> None:
                counter_key = 'no_retry'
            ...
            threads = [
                ...
                threading.Thread(target=_retry_immediately_inner, args=(start_barrier, end_barrier)),
            ]
